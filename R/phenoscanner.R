...
phenoscanner <- function(snpquery = NULL, genequery = NULL, regionquery = NULL, catalogue = "GWAS", pvalue = 1E-5, proxies = "None", r2 = 0.8, build = 37) {
  ...
  if (length(snpquery) > 0) {
    ...
    if ((length(snpquery) %% 10) > 0) {
      n_queries <- n_queries + 1
    }
    for (i in 1:n_queries) {
      if (i < n_queries) {
        qsnps <- paste0(snpquery[(1 + 10 * (i - 1)):(10 * i)], collapse = "+")
      } else {
        qsnps <- paste0(snpquery[(1 + 10 * (i - 1)):length(snpquery)], collapse = "+")
      }
    ...
    for (i in 1:n_queries) {
      ...
      if (length(json_data$results) > 0) {
        ...
        if (length(json_data$results) > 0) {
          ...
          print(paste0("Warning: no results found for ", genequery))
        }
      }
      if (length(json_data$genes) > 0) {
        fields_genes <- json_data$genes[[1]]
        json_data$genes[[1]] <- NULL
        if (length(json_data$genes) > 0) {
    ...
    for (i in 1:n_queries) {
      ...
      if (length(json_data$results) > 0) {
        ...
        if (length(json_data$results) > 0) {
          ...
          print(paste0("Warning: no results found for ", regionquery))
        }
      }
      if (length(json_data$locations) > 0) {
        fields_regions <- json_data$locations[[1]]
        json_data$locations[[1]] <- NULL
        if (length(json_data$locations) > 0) {
